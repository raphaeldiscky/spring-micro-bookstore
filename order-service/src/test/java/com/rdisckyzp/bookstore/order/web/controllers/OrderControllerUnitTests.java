...
import static com.rdisckyzp.bookstore.order.testdata.TestDataFactory.*;
...
class OrderControllerUnitTests {
    ...
    @BeforeEach
    void setUp() {
        given(securityService.getLoginUserName()).willReturn("raphael");
    }

    @ParameterizedTest(name = "[{index}]-{0}")
    @MethodSource("createOrderRequestProvider")
    void shouldReturnBadRequestWhenOrderPayloadIsInvalid(CreateOrderRequest request) throws Exception {
        given(orderService.createOrder(eq("raphael"), any(CreateOrderRequest.class)))
                .willReturn(null);
